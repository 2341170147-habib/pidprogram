/*
 * ESP32-S3 Chiller Alarm Monitoring with Firebase & Heartbeat
 ...
 * - Heartbeat every 10 minutes (for Last Sync detection)
 * - LCD display (ST7735)
...
// ============================================
const unsigned long HEARTBEAT_INTERVAL = 600000;  // 10 minutes = 600000ms
...
void setup() {
    ...
    Serial.println("\n\n=== ESP32-S3 Chiller Alarm Monitoring (v7 Heartbeat) ===\n");
...
void sendHeartbeat() {
    String ts = getTimestamp();
    String heartbeatPath = "/devices/" DEVICE_ID "/heartbeat";
    
    // Send heartbeat data
    Database.set<String>(aClient, heartbeatPath + "/timestamp", ts, processData, "RTDB_Heartbeat_Timestamp");
    Database.set<String>(aClient, heartbeatPath + "/status", "ALIVE", processData, "RTDB_Heartbeat_Status");
    Database.set<String>(aClient, heartbeatPath + "/type", "HEARTBEAT", processData, "RTDB_Heartbeat_Type");
    
    Serial.printf("[%s] 💓 Heartbeat sent to Firebase\n", ts.c_str());
