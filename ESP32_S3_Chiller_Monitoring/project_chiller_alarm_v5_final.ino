...
void sendAlarmAlert(uint8_t alarmNum, const char* status) {
    ...
    Database.set<object_t>(aClient, path.c_str(), json_obj, processData, "RTDB_Send_Alarm");
...
void sendWiFiStatusAlert(const char* status) {
    ...
    Database.set<object_t>(aClient, path.c_str(), json_obj, processData, "RTDB_Send_WiFi");
...
void createEventLog(const char* event) {
    ...
    String path = "/devices/" DEVICE_ID "/event_logs/" + pathKey;
    ...
    Database.set<object_t>(aClient, path.c_str(), json_obj, processData, "RTDB_Send_Log");
}
